...
class TimeSlotController < ApplicationController
  ...
  def suggested_timeslots
    ...
    # check if older reservation overlap and update the wanted date if yes
    overlapping = Timeslot.include_time(wanted_date).maximum('end_date')
    ...
    while @timeslots.length < 5
      ...
      if next_timeslots.length.positive?
        next_timeslot = next_timeslots.first
        unless Timeslot.overlaps(possible_start, possible_end, next_timeslot.start_date, next_timeslot.end_date)
